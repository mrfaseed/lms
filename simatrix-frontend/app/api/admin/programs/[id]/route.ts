...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const backendUrl = `${BACKEND_URL}/api/programs/${id}/`;
  ...
  try {
    ...
    if ((res.status === 401 || res.status === 403) && refresh) {
      ...
      if (!patchRes.ok) {
        const errorData = await patchRes.json().catch(() => ({ detail: 'Failed to update program' }));
    ...
    const errorData = await res.json().catch(() => ({ detail: 'Unable to update program' }));
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if ((res.status === 401 || res.status === 403) && refresh) {
      ...
      if (!delRes.ok) {
        const errorData = await delRes.json().catch(() => ({ detail: 'Failed to delete program' }));
    ...
    const errorData = await res.json().catch(() => ({ detail: 'Unable to delete program' }));
    return NextResponse.json(errorData, { status: res.status });
  } catch (err) {
    return NextResponse.json({ detail: 'Server error' }, { status: 500 });
  }
